...
from numpy import pi, sin, cos, linspace, sqrt, isnan

def triangle(x, y, size, color=(255, 255, 255, 150)):
    """Draws a triangle."""
...
def line(x0, y0, x1, y1, color=(255,255,255,255)):
    glPushMatrix()
    glColor4f(color[0]/255, color[1]/255, color[2]/255, color[3]/255)
    glLineWidth(3)
    glBegin(GL_LINES)
...
def cartesian(wband, wheight, delta):
    """Draws the arrows of x and y axises."""
    sz = delta/2
    glPushMatrix()
    glColor4f(0., 0., 0., 1.)
    #changes line width
...
def ContourLine(k, offset, delta):
    def func(x0, k):
        y2 = k - x0**2    # y^2 = k - x^2
        y0_plus = sqrt(y2)
        if not isnan(y0_plus):
            y0_minus = -y0_plus
            return y0_plus, y0_minus
        else:
            return None, None
    #
    for x0 in linspace(-10, 10, 5000):
        y0_plus, y0_minus = func(x0, k)
        if y0_plus is not None:
            circle(x0 * delta + offset, y0_plus * delta + offset, radius=2, color=(255,255,0))
        if y0_minus is not None:
            circle(x0 * delta + offset, y0_minus * delta + offset, radius=2, color=(255,255,0))
